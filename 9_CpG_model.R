...
#
# Output: the donor's age estimated by the 9-CpG model.
...
setwd(dirModel)
panel_9_CpG_model <- readRDS("9_CpG_svmPoly_model.rds")
...
setwd(dirResult)
write.csv(meth_data, "meth_data_panel_9_CpG.csv", quote = FALSE, row.names = FALSE)
...
# # ℹ 1 more variable: cg01789162 <dbl>

## Estimate age using the 9-CpG model ---------------------------------------
# Estimate age
estimated_age <- data.frame(
  Sample.Name = meth_data$Sample.Name,
  Estimated.Age = predict(panel_9_CpG_model, newdata = meth_data[,-1])
)
# Save estimated ages into a .csv file
setwd(dirResult)
write.csv(estimated_age, "estimated_age_panel_9_CpG.csv", quote = FALSE, 
          row.names = FALSE)
